...
const createUser = async (data, notification = UserNotification) => {
  try {
    const response = await responseFromServer(`${SchoolURL}/users`, null, notification, 'POST', data);
    return response;
  } catch (error) {
    const failResponse = {
      data: null,
...
const isAuthUser = async (data, notification = UserCreatedMsg) => {
  try {
    const response = await responseFromServer(`${SchoolURL}/signin`, null, notification, 'POST', data);
    return response;
  } catch (error) {
